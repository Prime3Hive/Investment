...
This migration creates the complete database schema for the investment platform including:

1. New Tables
   - `profiles` - User profile data with wallet addresses
   - `investment_plans` - Available investment plans
   - `investments` - User investments tracking
   - `deposits` - Deposit requests and confirmations
...
-- Create trigger for profiles updated_at (drop first if exists)
DROP TRIGGER IF EXISTS update_profiles_updated_at ON profiles;
CREATE TRIGGER update_profiles_updated_at
  BEFORE UPDATE ON profiles
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Insert default investment plans (use ON CONFLICT to avoid duplicates)
...
-- Add unique constraint on plan names if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'investment_plans_name_key' 
